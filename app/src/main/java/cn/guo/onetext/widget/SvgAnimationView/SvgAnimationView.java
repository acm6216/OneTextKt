...
public class SvgAnimationView extends View {
    ...
    public SvgAnimationView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SvgAnimationView);
            mViewportWidth = a.getInt(R.styleable.SvgAnimationView_animatedSvgImageSizeX, 512);
            aspectRatioWidth = a.getInt(R.styleable.SvgAnimationView_animatedSvgImageSizeX, 512);
            mViewportHeight = a.getInt(R.styleable.SvgAnimationView_animatedSvgImageSizeY, 512);
            aspectRatioHeight = a.getInt(R.styleable.SvgAnimationView_animatedSvgImageSizeY, 512);
            mTraceTime = a.getInt(R.styleable.SvgAnimationView_animatedSvgTraceTime, 2000);
            mTraceTimePerGlyph = a.getInt(R.styleable.SvgAnimationView_animatedSvgTraceTimePerGlyph, 1000);
            mFillStart = a.getInt(R.styleable.SvgAnimationView_animatedSvgFillStart, 1200);
            mFillTime = a.getInt(R.styleable.SvgAnimationView_animatedSvgFillTime, 1000);
            int traceMarkerLength = a.getInt(R.styleable.SvgAnimationView_animatedSvgTraceMarkerLength, 16);
            mMarkerLength =
                    TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, traceMarkerLength, getResources().getDisplayMetrics());
            int glyphStringsId = a.getResourceId(R.styleable.SvgAnimationView_animatedSvgGlyphStrings, 0);
            int traceResidueColorsId = a.getResourceId(R.styleable.SvgAnimationView_animatedSvgTraceResidueColors, 0);
            int traceColorsId = a.getResourceId(R.styleable.SvgAnimationView_animatedSvgTraceColors, 0);
            int fillColorsId = a.getResourceId(R.styleable.SvgAnimationView_animatedSvgFillColors, 0);
    ...
    public void startSvgDataResource(String assets_Resource) {
        List<PathModel> pathModelList = null;
        AssetManager asset = getContext().getAssets();
        try {
            InputStream input = asset.open(assets_Resource);
            // 创建一个解析xml的工厂对象
            ...
            input.close();
            // 获取解析出来的数据
            pathModelList = handler.getDataList();

            String[] pathss = new String[pathModelList.size()];
            int[] colorss = new int[pathModelList.size()];

            for (int i = 0; i < pathModelList.size(); i++) {
                pathss[i] = pathModelList.get(i).getPath();
                if (TextUtils.isEmpty(pathModelList.get(i).getColor())) {
                    colorss[i] = 0XFF000000;
                } else {
                    String color_pin = pathModelList.get(i).getColor();

                    colorss[i] = Color.parseColor(color_pin);
                }
            }
            setGlyphStrings(pathss);
            setTraceColors(colorss);
            setFillColors(colorss);
     ...
     *
     * @param onStateChangeListener The {@link OnStateChangeListener}
     */
